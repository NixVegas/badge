...
esp_err_t nixbadge_mesh_send_packet(const mesh_addr_t* to, uint8_t kind) {
    if (to != NULL) ESP_LOGI(TAG, "Sending to "MACSTR" with %d", MAC2STR(to->addr), kind);
...
static void esp_mesh_p2p_tx_main(void* arg) {
    ...
    while (is_running) {
        ...
        ESP_LOGW(TAG, "[TX-PENDING][L:%d]to_parent:%d, to_parent_p2p:%d, to_child:%d, to_child_p2p:%d, mgmt:%d, broadcast:%d",
            mesh_layer,
            pending.to_parent,
...
static void esp_mesh_p2p_rx_main(void* arg) {
    ...
    while (is_running) {
        ...
        ESP_LOGW(TAG, "[RX-PENDING][L:%d]ds:%d, self:%d", mesh_layer, pending.toDS, pending.toSelf);
        if (pending.toSelf < 1) {
